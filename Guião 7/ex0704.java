public class ex0704 {
	...
	public static void main (String args[]) {
		...
		setArrayFalse(board);
		if(solveSuperQueen(board, 0)) print(board);
		else System.out.print("Não foi encontrada solução.\n");
	}
	...
	public static boolean attacked(boolean[][] arr, int i, int j){
		...
		for(int k = 0; i+k < arr.length && j+k < arr.length; k++) if(arr[i+k][j+k]) return true;
		for(int k = 0; i-k >= 0 && j+k < arr.length; k++) if(arr[i-k][j+k]) return true;
		for(int k = 0; i+k < arr.length && j-k >= 0; k++) if(arr[i+k][j-k]) return true;
		if(i-2 >= 0 && j-1 >= 0 && arr[i-2][j-1]) return true;
		if(i-2 >= 0 && j+1 < arr.length && arr[i-2][j+1]) return true;
		if(i+2 < arr.length && j-1 >= 0 && arr[i+2][j-1]) return true;
		if(i+2 < arr.length && j+1 < arr.length && arr[i+2][j+1]) return true;
		if(i-1 >= 0 && j-2 >= 0 && arr[i-1][j-2]) return true;
		if(i-1 >= 0 && j+2 < arr.length && arr[i-1][j+2]) return true;
		if(i+1 < arr.length && j-2 >= 0 && arr[i+1][j-2]) return true;
		if(i+1 < arr.length && j+2 < arr.length && arr[i+1][j+2]) return true;
		return false;
	}
