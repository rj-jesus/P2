...
public class AssociativeArray<K extends Comparable<? super K>, T>{
	...
	public T get(K key){
		if(!exists(key)) throw new AssertionError("Error: Key not existent.");
	...
	private Node<K, T> delete(Node<K, T> node, K key){
		...
		else if(n == 0 && node.left == null) return node.right;
		else if(n == 0 && node.right == null) return node.left;
		else if(n == 0){
		    node.key = node.left.key;
		    node.value = node.left.value;
		    return delete(node.left, node.left.key);
		}
